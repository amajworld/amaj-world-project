...
export default function PostEditor({ initialPost }: { initialPost: Partial<Post> | null }) {
  ...
  const handleSave = async (isPublishOrSchedule: boolean) => {
    ...
    if (isPublishOrSchedule && post.status === 'draft') {
      finalStatus = 'published';
    }
    
    // When an existing draft is published/scheduled, also set its status
    ...
    try {
      if (post.id) {
        ...
        // This is a new post, so we need to add it.
        const newPostWithId = { ...postData, id: '' }; // id will be generated by firestoreActions
        await addDocument('posts', newPostWithId);
      }
